...
export async function applyPatch(
  ...
  try {
    const res = await fetch(
      `${BASE}/crm/v3/objects/${patch.objectType}/${patch.objectId}`,
      {
        method: "PATCH",
        headers: {
...
export async function applyBatch(
  ...
  if (patches.length > 100) throw new Error("Batch limit is 100 records per call");

  const inputs = patches.map((p) => ({ id: p.objectId, properties: p.properties }));

  try {
    const res = await fetch(
      `${BASE}/crm/v3/objects/${objectType}/batch/update`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
